function findCounter() {     
    ...
    let capitalizedUserType = userType.charAt(0).toUpperCase() + 
                                userType.slice(1);
                       
    capitalizedUserType = capitalizedUserType
    .replace(/[\W_]/gi, "")    
    ...
    arrayOfCounters.forEach((el) => {
        countersToCounters.innerHTML += "<b>" + el + "</b> is weak against: " + "<b>" + calculateCounter(el)[0] + "</b><br>";
    });

    if (counterFound) {
        answer.innerHTML = "<h3>" + capitalizedUserType + " is weak against: <br></h3>" + goodType ;
    } else {
        answer.innerHTML = goodType + " Are you sure you spelled the type correctly?";
    }
}
